...
df_h=Sl_pw  ##### For all hours analysis
...
df_h$S1=runmax(df_h$S1,336)  ###For all hours along the week
df_h$S2=runmax(df_h$S2,336)  ###For all hours along the week
###############
...
############ X-axis of time series with date instead numerical series
ggplot(df_h_orig, aes(Date, S2)) + geom_line(color='green') +
geom_line(aes(y=yp$y, col=yp$x), color='red', lwd = 1)+
  labs(title="Time Series of PV Solar Power",
       subtitle="PW-S2",
       caption="",
